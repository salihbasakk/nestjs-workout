...
export class CarService {
  ...
  public async getCars(): Promise<CarDto[]> {
    const cars = await this.carModel.find({}, carsProjection).exec();
    if (!cars || !cars[0]) {
  ...
  public async getCarById(id: number): Promise<CarDto> {
    const car = await this.carModel.findOne({ id }, carsProjection).exec();
    if (!car) {
      throw new HttpException('Not Found', 404);
    }
    return car;
  ...
  public async deleteCarById(id: number): Promise<string> {
